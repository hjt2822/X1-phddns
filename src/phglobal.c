...
static void defOnStatusChanged(PHGlobal* global, int status, int data)
{
	...
	if (status == okDomainsRegistered)
	{
		...
		/*
		FILE *fp=fopen("/web/jffs2/phddns_statu.conf","w+");	
		if(fp == NULL)
			return;
		fputs("1",fp);
		fclose(fp);
		*/
		system("echo '1'>/web/jffs2/phddns_statu.conf");
	}
	if(status == okRedirecting)
	{
		//FILE *fp1=fopen("/web/phddns_enable.conf","w+");	
		//fputs("1",fp1);
		//fclose(fp1);
		system("echo '1'>/web/phddns_enable.conf");
	}
	if(status == errorConnectFailed)
	{
		system("echo '0'>/web/phddns_enable.conf");
		//FILE *fp11=fopen("/web/phddns_enable.conf","w+");	
		//fputs("0",fp11);
		//fclose(fp11);
	}
	if(status == errorKeepAliveError)
	{
		//FILE *fp2=fopen("/web/phddns_onoff.conf","w+");	
		//if(fp2 == NULL)
		//	return;
		//fputs("0",fp2);
		//fclose(fp2);
		system("echo '0'>/web/phddns_onoff.conf");
	}
	printf("\n");
}
...
static void defOnUserInfo(PHGlobal* global, char *userInfo, int len)
{
	...
	FILE *ufp=fopen("/web/phddns_userinfo.xml","w+");
	if(ufp == NULL)
		return;
	fputs(userInfo,ufp);
	fclose(ufp);
...
static void defOnAccountDomainInfo(PHGlobal* global, char *domainInfo, int len)
{
	...
	FILE *dfp=fopen("/web/phddns_domaininfo.conf","w+");
	if(dfp == NULL)
		return;
	fputs(buf,dfp);
	fclose(dfp);
